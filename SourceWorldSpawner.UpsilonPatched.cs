...
namespace SpiritsCrossing.SourceWorld
{
    ...
    public class SourceWorldSpawner : MonoBehaviour
    {
        ...
        [Header("Upsilon Source Layer")]
        public bool  enableUpsilonSourceLayer = true;
        [Range(0f, 1f)] public float discoveryPulseStrength = 0.18f;
        ...
        private UpsilonSourceLayer _upsilonLayer;
        ...
        private void Update()
        {
            ...
            if (enableUpsilonSourceLayer)
            {
                _upsilonLayer ??= UpsilonSourceLayer.Instance ?? FindObjectOfType<UpsilonSourceLayer>();
                _upsilonLayer?.TickLayer(Time.deltaTime, this);
            }
        }
        ...
        public void EnterZone(string zoneId)
        {
            ...
            {
                _upsilonLayer ??= UpsilonSourceLayer.Instance ?? FindObjectOfType<UpsilonSourceLayer>();
                _upsilonLayer?.EnterSourceZone(zone, this);
            }
        ...
        private void ScanZone(string zoneId)
        {
            ...
            foreach (var chamber in zone.communionChambers)
            {
                    ...
                    h >= chamber.outerDiscoveryThreshold * ts)
                {
                    ...
                    _upsilonLayer?.RegisterDiscoveryPulse(discoveryPulseStrength * 0.90f, "communion_outer");
                    ...
                    h >= chamber.innerDiscoveryThreshold * ts)
                {
                    ...
                    _upsilonLayer?.RegisterDiscoveryPulse(discoveryPulseStrength, "communion_inner");
            ...
            foreach (var well in zone.originWells)
            {
                ...
                if (playerField.WeightedHarmony(well.frozenField) >= well.discoveryThreshold * ts)
                {
                    ...
                    _upsilonLayer?.RegisterDiscoveryPulse(discoveryPulseStrength * 1.40f, "origin_well");
        ...
        private void TickPassiveFields(string zoneId)
        {
            ...
            foreach (var pool in zone.veilPools)
            {
                ...
                if (!_touchedPools.Contains(pool.poolId))
                {
                    ...
                    _upsilonLayer?.RegisterDiscoveryPulse(discoveryPulseStrength * 0.75f, "veil_pool");
            ...
            foreach (var gate in zone.thresholdGates)
            {
                ...
                if (allMet)
                {
                    ...
                    if (_gateTimers[gate.gateId] >= gate.sustainDurationSeconds)
                    {
                        ...
                        _upsilonLayer?.RegisterDiscoveryPulse(discoveryPulseStrength * 1.20f, "threshold_gate");
        ...
        public SourceWorldZone GetActiveZone() => string.IsNullOrEmpty(ActiveZoneId) ? null : Data?.GetZone(ActiveZoneId);
        ...
        public float ZoneCompletion01()
        {
            ...
            return Mathf.Clamp01((float)DiscoveryCount() / total);
        }
        ...
        public float AverageGateProgress01()
        {
            ...
            foreach (var gate in zone.thresholdGates)
            {
                ...
                if (_gateTimers.TryGetValue(gate.gateId, out float timer) && gate.sustainDurationSeconds > 0f)
                    sum += Mathf.Clamp01(timer / gate.sustainDurationSeconds);
            }
            return Mathf.Clamp01(sum / zone.thresholdGates.Count);
        }

        public float SourceDiscoveryBias()
        {
            float pools = _touchedPools.Count * 0.20f;
            float chambers = (_discoveredChamberOuters.Count + _discoveredChamberInners.Count) * 0.25f;
            float gates = _completedGates.Count * 0.40f;
            float wells = _discoveredWells.Count * 0.60f;
            return Mathf.Clamp01(pools + chambers + gates + wells);
        }
    }
}
